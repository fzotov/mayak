...
function Badge({ s }: { s: string }) {
  ...
  return <span style={{ padding: '2px 8px', borderRadius: 10, fontSize: 13, fontWeight: 500, background: b.bg, color: b.color }}>{b.label}</span>
}

function KpiCard({ label, value, sub, color }: { label: string; value: string | number; sub?: string; color?: string }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #e8ebf3', borderRadius: 9, padding: '11px 13px' }}>
      <div style={{ fontSize: 12, color: '#8596b4', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.4px' }}>{label}</div>
      <div style={{ fontSize: 20, fontWeight: 600, color: color ?? '#1a2240', lineHeight: 1, marginBottom: 4 }}>{value}</div>
      {sub && <div style={{ fontSize: 12, color: '#8596b4' }}>{sub}</div>}
    </div>
  )
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div style={{ background: '#fff', border: '1px solid #e8ebf3', borderRadius: 9, padding: '13px 15px' }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#1a2240', marginBottom: 10 }}>Задачи на сегодня</div>
          {mockTasks.map(t => (
            <div key={t.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '7px 0', borderBottom: '1px solid #f0f2f8' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 500, color: '#1a2240' }}>{PRIORITY_PREFIX[t.priority]}{t.title}</div>
                <div style={{ fontSize: 12, color: '#8596b4', marginTop: 2 }}>{t.source === 'OWNER' ? 'от руководителя' : t.source === 'SYSTEM' ? 'авто' : 'личная'}</div>
              </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #e8ebf3', borderRadius: 9, padding: '13px 15px' }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#1a2240', marginBottom: 10 }}>Просроченные счета</div>
          {mockOverdue.map(inv => (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 8, marginTop: 4, borderTop: '1px solid #f0f2f8' }}>
            <span style={{ fontSize: 12, color: '#8596b4' }}>Итого долг</span>
      ...
      <div style={{ background: '#fff', border: '1px solid #e8ebf3', borderRadius: 9, padding: '13px 15px' }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#1a2240', marginBottom: 10 }}>Ближайшие события</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 24px' }}>
          {mockEvents.map(e => (
            <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: '1px solid #f0f2f8' }}>
              ...
              <div style={{ fontSize: 12, color: '#8596b4', width: 48, flexShrink: 0 }}>{e.eventDate.slice(5).replace('-', ' ')}</div>
              <div style={{ fontSize: 13, color: '#374151' }}>{e.title}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
        {['', 'DRAFT', 'SENT', 'OVERDUE', 'PAID'].map(s => (
          <button key={s} onClick={() => setFilter(s)}
            style={{ padding: '5px 12px', borderRadius: 6, border: '1px solid', fontSize: 14, cursor: 'pointer', fontFamily: 'inherit',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
      {mockTasks.map(t => (
        <div key={t.id} style={{ background: '#fff', border: '1px solid #e8ebf3', borderRadius: 9, padding: '12px 15px', display: 'flex', alignItems: 'center', gap: 10, borderLeft: `3px solid ${SOURCE_COLOR[t.source]}` }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 500, color: '#1a2240' }}>{PRIORITY_PREFIX[t.priority]}{t.title}</div>
            <div style={{ fontSize: 13, color: '#8596b4', marginTop: 3 }}>
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f0f2f8', fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif' }}>
      <aside style={{ width: 200, flexShrink: 0, background: '#1e2235', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '12px 14px', borderTop: '1px solid #ffffff12', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 500, color: '#c9cdd8' }}>Андрей Соколов</div>
