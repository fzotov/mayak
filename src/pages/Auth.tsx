      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: 40, width: 380, border: '1px solid #e8ebf3' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: '#8596b4' }}>Property OS</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', background: '#f0f2f8', borderRadius: 8, padding: 3, marginBottom: 24, gap: 3 }}>
          {(['login', 'register'] as const).map(m => (
            <button key={m} onClick={() => setMode(m)}
              style={{ flex: 1, padding: '7px', borderRadius: 6, border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 500,
        ...
        <form onSubmit={handleSubmit}>
          {mode === 'register' && (
            <>
              <label style={{ fontSize: 14, fontWeight: 500, color: '#374151', display: 'block', marginBottom: 5 }}>Имя</label>
              <input style={inp} value={name} onChange={e => setName(e.target.value)} placeholder="Иван Иванов" required />
            </>
          )}
          <label style={{ fontSize: 14, fontWeight: 500, color: '#374151', display: 'block', marginBottom: 5 }}>Email</label>
          <input style={inp} type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="email@example.com" required />
          <label style={{ fontSize: 14, fontWeight: 500, color: '#374151', display: 'block', marginBottom: 5 }}>Пароль</label>
          ...
          {error && <div style={{ fontSize: 14, color: '#ef4444', marginBottom: 12, padding: '8px 12px', background: '#fef2f2', borderRadius: 6 }}>{error}</div>}
          {message && <div style={{ fontSize: 14, color: '#16a34a', marginBottom: 12, padding: '8px 12px', background: '#f0fdf4', borderRadius: 6 }}>{message}</div>}

          <button type="submit" disabled={loading}
            style={{ width: '100%', padding: 11, background: '#4f6ef7', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>
